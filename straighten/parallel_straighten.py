...
if __name__ == '__main__':
    ...
    # parser.add_argument('--image_path', default='C:/Users/Yanfeng Zhou/Desktop/zhongshan_v3/zhongshan_v1_image')
    # parser.add_argument('--seg_path', default='C:/Users/Yanfeng Zhou/Desktop/zhongshan_v3/zhongshan_v1_seg')
    # parser.add_argument('--centerline_path', default='C:/Users/Yanfeng Zhou/Desktop/zhongshan_v3/zhongshan_v1_target_vessel')
    # parser.add_argument('--save_straighten_img_path', default='C:/Users/Yanfeng Zhou/Desktop/zhongshan_v3/straighten_image')
    # parser.add_argument('--save_straighten_seg_path', default='/mnt/sdb1/zhouyanfeng.zyf/zhongshan_v3/straighten_seg')
    parser.add_argument('--image_path', default='/mnt/sdb1/zhouyanfeng.zyf/zhongshan_v3/zhongshan_v1_image')
    parser.add_argument('--seg_path', default='/mnt/sdb1/zhouyanfeng.zyf/zhongshan_v3/zhongshan_v1_seg_refine')
    parser.add_argument('--centerline_path', default='/mnt/sdb1/zhouyanfeng.zyf/zhongshan_v3/zhongshan_v1_refine_target_vessel')
    parser.add_argument('--save_straighten_img_path', default='/mnt/sdb1/zhouyanfeng.zyf/zhongshan_v3/straighten_image_r32')
    parser.add_argument('--save_straighten_seg_path', default='/mnt/sdb1/zhouyanfeng.zyf/zhongshan_v3/straighten_seg_r32')
    ...
    # for i in os.listdir(args.centerline_path):
    #
    #     centerline = np.load(os.path.join(args.centerline_path, i), allow_pickle=True).item()
    #     name = os.path.splitext(i)[0] + '.gz'
    #
    #     image = sitk.ReadImage(os.path.join(args.image_path, name))
    #     image = sitk.GetArrayFromImage(image)
    #
    #     seg = sitk.ReadImage(os.path.join(args.seg_path, name))
    #     seg = sitk.GetArrayFromImage(seg)
    #
    #     for j in centerline:
    #         point_list = centerline[j]['point']
    #         save_straighten_name = os.path.splitext(i)[0] + '_' + str(j) + '.tif'
    #         # print(i, j, centerline[j]['edge_width'], centerline[j]['edge_length'])
    #
    #         for k in range(len(point_list)):
    #
    ...
    #                 coordinate = np.concatenate((coordinate, np.array([[x, y, z]])), axis=0)
    #
    #         if args.remove_duplicate_point:
    #             unique_mask = np.any(coordinate[1:] != coordinate[:-1], axis=1)
    #             unique_mask = np.concatenate([[True], unique_mask])
    #             coordinate = coordinate[unique_mask]
    #
    ...
    #
    #             # coordinate matrix in new coordinate system
    #             coordinate_matrix_one = np.linspace(-args.crop_radius, args.crop_radius, 2 * args.crop_radius + 1)
    #             coordinate_matrix_one = np.tile(coordinate_matrix_one, 2 * args.crop_radius + 1)
    #             coordinate_matrix_two = np.arange(args.crop_radius, -args.crop_radius - 1, -1)
    #             coordinate_matrix_two = np.repeat(coordinate_matrix_two, 2 * args.crop_radius + 1)
    #             coordinate_matrix_three = np.zeros((2 * args.crop_radius + 1) ** 2)
    #             coordinate_matrix_four = np.ones((2 * args.crop_radius + 1) ** 2)
    #             coordinate_matrix = np.stack((coordinate_matrix_one, coordinate_matrix_two, coordinate_matrix_three, coordinate_matrix_four), axis=0)
    #
    #             coordinate_matrix_ori = np.dot(R, coordinate_matrix)
    #             coordinate_matrix_ori = coordinate_matrix_ori[:3, :]
    #
    #             gray_value_img = ndimage.map_coordinates(image, coordinate_matrix_ori, order=3)
    #             gray_value_img = gray_value_img.reshape(2 * args.crop_radius + 1, 2 * args.crop_radius + 1)
    #
    #             gray_value = ndimage.map_coordinates(seg, coordinate_matrix_ori, order=1)
    #             gray_value = gray_value.reshape(2 * args.crop_radius + 1, 2 * args.crop_radius + 1)
    #
    #             if m == 0:
    #                 straighten_seg = np.array([gray_value])
    #                 straighten_img = np.array([gray_value_img])
    ...
    #         straighten_seg = straighten_seg.astype(bool)
    #         # fill hole
    #         straighten_seg = remove_small_holes(straighten_seg, args.remove_small_holes_thr)
    #         # remove small object
    #         straighten_seg = save_center_objects(straighten_seg, 2 * args.crop_radius + 1, 2 * args.crop_radius + 1, int(centerline[j]['edge_length']))
    #         # smooths
    #         if args.if_smooth:
    #             straighten_seg = ndimage.median_filter(straighten_seg, size=3)
    #
    #         straighten_seg = sitk.GetImageFromArray(straighten_seg.astype(np.uint8))
    #         sitk.WriteImage(straighten_seg, os.path.join(args.save_straighten_seg_path, save_straighten_name))
    #
    #         straighten_img = sitk.GetImageFromArray(straighten_img.astype(np.int16))
    #         sitk.WriteImage(straighten_img, os.path.join(args.save_straighten_img_path, save_straighten_name))
